...
class WalmartReviewScraper:
    ...
    def scrape_all_pages(self, url: str) -> List[Dict]:
        """
        Scrape ALL reviews from all pages without limit
        """
        ...
        max_consecutive_failures = 2  # Reduced to stop sooner
        seen_reviews = set()  # Track unique reviews to detect loops
        ...
        while True:
            try:
                ...
                # The URL format for reviews is: /reviews/product/ID?page=X
                ...
                if not page_reviews:
                    ...
                    print(f"   ⚠️  No reviews found on page {page} (attempt {consecutive_failures}/{max_consecutive_failures})")
                    
                    if consecutive_failures >= max_consecutive_failures:
                        print(f"\n   📌 No more reviews found after {consecutive_failures} consecutive empty pages. Stopping.")
                        break
                else:
                    # Check for duplicate reviews (indicates we're seeing same data)
                    new_review_count = 0
                    for review in page_reviews:
                        review_key = (review.get('review_text', ''), review.get('reviewer_name', ''))
                    ...
                    if new_review_count == 0:
                        consecutive_failures += 1
                        print(f"   ⚠️  All reviews on page {page} are duplicates (attempt {consecutive_failures}/{max_consecutive_failures})")
                        if consecutive_failures >= max_consecutive_failures:
                            print(f"\n   📌 No new reviews found. Stopping.")
                            break
                    else:
                        consecutive_failures = 0  # Reset counter on success
                        print(f"   ✅ Found {len(page_reviews)} reviews ({new_review_count} new)")
                
                # Check if there's a next page button
                next_button = soup.find('button', {'aria-label': 'next page'}) or \
                             soup.find('a', {'aria-label': 'Go to next reviews page'})
                
                if next_button:
                    # Check if button is disabled
                    is_disabled = (
                        'disabled' in next_button.get('disabled', '') or
                        'disabled' in next_button.get('class', []) or
                        next_button.get('aria-disabled') == 'true'
                    )
                ...
                # Look for pagination info to determine if we should continue
                pagination_info = soup.find('div', class_=re.compile(r'pagination', re.I))
                if pagination_info and page > 1:
                    # Check if current page is the last
                    if f"page {page}" in pagination_info.text.lower() and "of" in pagination_info.text.lower():
                        try:
                            total_pages = int(re.search(r'of\s+(\d+)', pagination_info.text, re.I).group(1))
                            if page >= total_pages:
                                print(f"\n   📌 Reached last page ({page} of {total_pages})")
                                break
                        except:
                            pass
    ...
    def extract_reviews_from_json(self, data: Dict, reviews: List = None) -> List[Dict]:
        ...
        if isinstance(data, dict):
            ...
            for key in ['reviews', 'customerReviews', 'reviewList', 'ReviewsList']:
...
def main():
    ...
    try:
        # Scrape ALL reviews
        print("\n🔍 Starting scraping process (ALL PAGES)...\n")
        reviews = scraper.scrape_all_pages(product_url)
